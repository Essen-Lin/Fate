...
## 讀取需要的資料(excel)
input_path = '../input/tscs992.csv'
input_data = pd.read_csv(input_path, encoding='utf-8', usecols=['id','marital_status'])
...
class MarryID:
    ...
    def generate_love_rule(self):
        ...
        result=[]
        # print('ID: ', ID)
        id_input = input_data.index[input_data['id']==int(ID)].to_list()
        marital_status = input_data['marital_status'][id_input].to_list()[0]
...
if __name__ == '__main__':
    Dir_path = glob.glob(os.path.join("../data/tscs992",'*'))
    ID_list =[]
    ID_date =[]
    for i in Dir_path:
    ...
    result_col = ['ID','大限','受訪時大限','結婚狀態','Rule1','Rule2','Rule3','Rule4','Rule5','Rule6','Rule7','Rule8','Rule9','Rule10',
                        'Rule11','Rule12','Rule13','Rule14','Rule15','Rule16','Rule17','Rule18','Rule19','Rule20']

    result = pd.DataFrame([result_col])
    # len(ID_path)
    for i in range(len(ID_path)): 
        path = '../data/tscs992/'+str(ID_path[i][0])+'-'+ID_path[i][1]
        Obj = MarryID(path)
    ...
    result.to_csv('/Users/essen/LocalData/NCTU/Fate/data/love_rule_tscs992_marry.csv',encoding='Big5', index=False,header=False)